...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
            # messages.success(request, 'We send verefication mail to you email address.')
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            try:
                query = requestsLib.utils.urlparse(url).query
...
def dashboard(request):
    ...
    context={'orders_count':orders_count,'profile':profile}
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Passowrd reset email has been sent.')
